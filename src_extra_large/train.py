...
from src_extra_large.dataset import GoDataset
from src_extra_large.model import TinyGoNet
...
def train(epochs=10, batch_size=64, lr=0.001, limit=0, device='cpu'):
    ...
    start_epoch = 0
    best_acc = 0.0
    if os.path.exists("best_model_extra_large.pth"):
        print("Resuming from best_model_extra_large.pth...")
        state = torch.load("best_model_extra_large.pth", map_location=device)
