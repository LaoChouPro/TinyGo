...
from src.inference import load_model, predict_move

class GoGui:
    def __init__(self, root, model_path):
        self.root = root
        self.root.title("TinyGo AI - Play against Best Model")
    ...
    def undo_move(self):
        ...
        if self.game_over:
            # Allow undo even if game over? Maybe.
            pass
        ...
        if 0 <= r < self.board_size and 0 <= c < self.board_size:
            ...
            if self.board.board[r, c] == EMPTY:
                ...
                if res != -1:
                    ...
                    # Pop state if move invalid? No, we saved, but play failed, so board didn't change.
                    # But if play failed, we should probably pop the history we just pushed?
                    # Or just check result first?
                    # Move validation is cheap. play() does it.
                    # If play returns -1, we should pop history.
                    self.history.pop()
    ...
    def human_pass(self):
        ...
        # Pass is essentially skipping turn, but we need to handle logic properly.
        # For now, just swap turn.
